...
figure;
subplot(221);imagesc(xticks,yticks,abs(image_upsample));title('(a)放大后的点目标');xlabel('距离向（采样点）');ylabel('方位向（采样点）');
subplot(222);contour(xticks,yticks,abs(image_upsample),30);title('(b)放大后的点目标等值线图');set(gca,'YDir','reverse');xlabel('距离向（采样点）');ylabel('方位向（采样点）');
subplot(223);plot(xticks*delta_r_new,20*log10(abs(signal_r)/max(abs(signal_r(:)))));xlim([0,size_r*delta_r_new]);ylim([-35,0]);title('(c)距离剖面图');xlabel('距离向(m)');ylabel('幅度(dB)');
subplot(224);plot(yticks*delta_a_new,20*log10(abs(signal_a)/max(abs(signal_a(:)))));xlim([0,size_a*delta_a_new]);ylim([-35,0]);title('(d)方位剖面图');xlabel('方位向(m)');ylabel('幅度(dB)');
end