...
sgtitle([num2str(theta_rc_deg), '度斜视角情况下的', num2str(NUM_TARGETS),'点雷达原始仿真信号（时域）']);
...
sgtitle('21.9度斜视角距离压缩后信号（时域与距离多普勒域）');
...
sgtitle('21.9度斜视角二次距离压缩前后信号对比（距离多普勒域）');
...
sgtitle('21.9度斜视角距离徙动校正前信号（距离多普勒域）');
...
sgtitle('21.9度斜视角距离徙动校正后信号（距离多普勒域）');
