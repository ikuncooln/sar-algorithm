...
%   
%   Note: 调用此函数相比于分别调用first_order_phase_error_compensation()和
%   range_resample()函数更加高效
c = 299792458;
% parameters convert
...
for k = 1:iter
    ...
    % 一阶相位误差补偿
    hc = exp(1j * 4 * pi * delta_R((k-1)*batch_size+(1:bs), :) / lambda);
    s0 = s0 .* hc;
